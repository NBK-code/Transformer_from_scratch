...
class PositionalEncoding(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.dropout(x + pe) # (batch, seq_len, d_model)
...
class MultiHeadAttentionBlock(nn.Module):
    ...
    def attention(query, key, value, mask, dropout: nn.Dropout):
        ...
        #The mask must be broadcastable to the attention score shape (batch, h, q_len, k_len)
        if mask is not None:
            attention_scores = attention_scores.masked_fill(mask == 0, -1e9)
        attention_scores = attention_scores.softmax(dim=-1) #dim = -1 gives probability distribution over keys
